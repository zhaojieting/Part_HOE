...
class PartHOEDecoder(nn.Module):
    def __init__(self):
        super(PartHOEDecoder, self).__init__()
        # 14*14*768 -> 56*56*17
        self.conv_for_heatmap1 = nn.ConvTranspose2d(384, 256, kernel_size=4, stride=2, padding=1,bias=False)
        self.bn1 = nn.BatchNorm2d(256)
        self.relu1 = nn.ReLU(inplace=True)
        self.conv_for_heatmap2 = nn.ConvTranspose2d(256, 256, kernel_size=4, stride=2, padding=1,bias=False)
        self.bn2 = nn.BatchNorm2d(256)
        self.relu2 = nn.ReLU(inplace=True)

        self.conv_for_conf1 = nn.Conv2d(384, 128, kernel_size=4, stride=2, padding=1,bias=False)
        self.conf_bn1 = nn.BatchNorm2d(128)
        self.conf_relu1 = nn.ReLU(inplace=True)
        self.conv_for_conf2 = nn.Conv2d(128, 128, kernel_size=4, stride=2, padding=1,bias=False)
        self.conf_bn2 = nn.BatchNorm2d(128)
        self.conf_relu2 = nn.ReLU(inplace=True)
        self.conf_pool = nn.AdaptiveAvgPool2d((1,1))
        self.conf_head = nn.Linear(128, 1)
        self.conf_sig = nn.Sigmoid()

        self.kpts_final = nn.Conv2d(256, 23, kernel_size=1, stride=1, padding=0)
...
class ViT(BaseBackbone):
    ...
    def __init__(self,
                 ...
                 frozen_stages=9, ratio=1, last_norm=True,
                 patch_padding='pad', freeze_attn=False, freeze_ffn=True,
                 ):
