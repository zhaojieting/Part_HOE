...
class FPN(nn.Module):
    def __init__(self,in_channel_list,out_channel):
        ...
        self.inner_layer = nn.ModuleList(self.inner_layer)
        self.out_layer = nn.ModuleList(self.out_layer)
...
from models.modules.replknet import KeypointsDecoder
from models.modules.replknet import HoeDecoder
from models.modules.replknet import DownsampleFeature
...
class FastViT(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.fpn_net = FPN([76,152,304,608], 256)
        self.kpt_decoder = KeypointsDecoder()
        self.hoe_decoder= HoeDecoder()
        self.freeze_network = True
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        feature = self.fpn_net(x)
        kpts, mask = self.kpt_decoder(feature[0])
        hoe, conf = self.hoe_decoder(x[-1])

        return kpts, hoe, conf, mask
